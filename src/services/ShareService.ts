...
export async function shareCard(game: PhaserGame, quote: string): Promise<void> {
    // Use cached Blob if available and for the same quote
    if (_cachedBlob !== null && _cachedQuote === quote) {
        // ── Cached path: safe to call navigator.share() synchronously ────────
        // The Blob was pre-generated by prepareShareCard() before the share button
        // appeared, so there is no async gap between the user gesture and share().
    ...
    // ── Cold-cache path (WR-04 fix) ─────────────────────────────────────────
    // Cache miss — there is an async gap before navigator.share() would be
    // called, which breaks the iOS user-gesture requirement (Pitfall 4).
    // On this path we skip navigator.share() entirely and go straight to the
    // download anchor, which has no synchronous-gesture requirement.
    const img    = await takeSnapshot(game);
    const canvas = renderShareCard(img, quote, null);
    const blob   = await canvasToBlob(canvas);
    ...
    // Cache for the next tap (so subsequent taps use the fast cached path)
    _cachedBlob  = blob;
    _cachedQuote = quote;

    // Download-only — do NOT attempt navigator.share() after awaits on iOS
    const url = URL.createObjectURL(blob);
    const a   = document.createElement('a') as HTMLAnchorElement;
    a.href     = url;
    a.download = 'barren-wuffett-victory.png';
